...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
    ...
    JOIN
      routes b ON (a.company = b.company AND a.num = b.num)
    JOIN
      stops transfer_stop ON (b.stop_id = transfer_stop.id)
    JOIN
      routes from_transfer ON (from_transfer.stop_id = transfer_stop.id)
    JOIN
      routes finish ON (from_transfer.company = finish.company AND finish.num = from_transfer.num)
    WHERE
      a.stop_id = (
        SELECT
          id
